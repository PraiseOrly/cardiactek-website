...
export function DiagnosticCenter() {
  ...
  return (
    <Section id="diagnostics" className="bg-gray-900 text-white">
      <motion.div
        ref={ref}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full">
                ...
                <div className="lg:col-span-2 bg-gray-900 p-6 relative overflow-hidden flex flex-col border border-white/10 rounded-2xl shadow-xl">
                  <div className="flex justify-between items-center mb-6">
                    <div className="text-white font-mono text-xs">LEAD II - CONTINUOUS</div>
                ...
                <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="flex flex-col gap-4">
                  <motion.div variants={scaleIn} className="bg-red-950/50 backdrop-blur-sm border-l-4 border-red-600 p-4 rounded-2xl">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-bold text-red-200 text-sm uppercase">STEMI Alert</h4>
                      <span className="bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">Priority Red</span>
                    </div>
                    <p className="text-red-300 text-xs mb-3">Inferior wall myocardial infarction suspected. ST elevation in II, III, aVF.</p>
                    <div className="text-right text-[10px] font-mono text-red-400">T-minus 2m 30s</div>
                  </motion.div>

                  <motion.div variants={scaleIn} className="bg-yellow-950/50 backdrop-blur-sm border-l-4 border-yellow-600 p-4 rounded-2xl">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="font-bold text-yellow-200 text-sm uppercase">Arrhythmia Detected</h4>
                      <span className="bg-yellow-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">Warning</span>
                    </div>
                    <p className="text-yellow-300 text-xs">Atrial Fibrillation with rapid ventricular response.</p>
                  </motion.div>
                  ...
                  <motion.div variants={scaleIn} className="bg-gray-800/80 backdrop-blur-sm border border-white/10 p-4 flex-1 flex flex-col justify-between rounded-2xl">
                    <div>
                      <h4 className="font-bold text-white text-sm uppercase mb-4">AI Analysis</h4>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs text-gray-400">Confidence Score</span>
                        <span className="text-lg font-bold text-red-400">98.7%</span>
                      </div>
...
export function DiagnosticTools() {
  ...
  return (
    <Section id="tools" className="bg-gray-800 border-t border-white/10 relative overflow-hidden">
      ...
      <motion.div ref={ref} initial={{ opacity: 0, y: 50 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="relative z-10">
        <div className="mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">ADVANCED DIAGNOSTIC TOOLS</h2>
...
export function CareCoordination() {
  ...
  return (
    <Section id="coordination" className="bg-gray-900 text-white">
      <motion.div ref={ref} initial={{ opacity: 0, y: 50 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
        <div className="mb-16 max-w-3xl">
          <h2 className="text-4xl md:text-5xl font-bold mb-4">CARE COORDINATION & WORKFLOW</h2>
          <p className="text-gray-300 text-lg italic">Streamline communication and data flow between patients, cardiologists, and care teams.</p>
        </div>
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? 'visible' : 'hidden'} className="grid grid-cols-1 md:grid-cols-2 gap-px bg-white/10 border border-white/10 rounded-3xl overflow-hidden">
          {features.map((feature, idx) => (
            <motion.div key={idx} variants={scaleIn} whileHover={{ scale: 1.02 }} className="bg-gray-800/80 backdrop-blur-sm p-10 hover:bg-gray-700/80 transition-all cursor-pointer">
              ...
              <p className="text-gray-300 mb-6 text-sm italic">{feature.description}</p>
...
export function PatientMonitoring() {
  ...
  return (
    <Section id="monitoring" className="bg-gray-800 border-t border-white/10 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div animate={{ y: [0, -30, 0], x: [0, 20, 0] }} transition={{ duration: 8, repeat: Infinity }} className="absolute top-1/4 left-1/4 w-96 h-96 bg-red-500/20 rounded-full blur-3xl" />
        <motion.div animate={{ y: [0, 40, 0], x: [0, -30, 0] }} transition={{ duration: 10, repeat: Infinity }} className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-blue-600/10 rounded-full blur-3xl" />
        <motion.div animate={{ y: [0, -20, 0], x: [0, 15, 0] }} transition={{ duration: 12, repeat: Infinity }} className="absolute top-1/2 right-1/3 w-64 h-64 bg-purple-600/10 rounded-full blur-3xl" />
      ...
      <motion.div ref={ref} initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 0.8 }} className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative z-10">
        <motion.div initial={{ opacity: 0, x: -50 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }} className="lg:col-span-4">
          ...
          <p className="text-gray-300 mb-8 italic">Extend care beyond the clinic walls with intelligent tracking and engagement tools.</p>
          <motion.div whileHover={{ scale: 1.05 }} className="p-6 border border-white/10 bg-gray-900/80 backdrop-blur-sm rounded-2xl shadow-xl">
            <motion.div initial={{ scale: 0 }} animate={isInView ? { scale: 1 } : {}} transition={{ duration: 0.6, delay: 0.3 }} className="text-5xl font-bold text-white mb-2">
              73%
            </motion.div>
            <div className="text-sm text-gray-400 uppercase tracking-wider">Reduction in 30-day readmissions</div>
          </motion.div>
        </motion.div>
        ...
        <div className="lg:col-span-8">
          <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? 'visible' : 'hidden'} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {features.map((feature, idx) => (
              <motion.div key={idx} variants={scaleIn} whileHover={{ scale: 1.05, y: -5 }} className="border border-white/10 bg-gray-900/80 backdrop-blur-sm p-8 hover:border-red-600/50 transition-all group cursor-pointer rounded-3xl shadow-xl">
                ...
                <a href="#" className="inline-flex items-center text-xs font-bold uppercase tracking-widest text-gray-400 group-hover:text-red-400 transition-colors">
...
export function PerformanceAnalytics() {
  ...
  return (
    <Section id="analytics" className="bg-gray-900 text-white">
      <motion.div ref={ref} initial={{ opacity: 0, y: 50 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
        <div className="mb-16 text-center">
          <h2 className="text-4xl md:text-5xl font-bold mb-4">PERFORMANCE & OUTCOMES ANALYTICS</h2>
          <p className="text-gray-300 italic">Quantify your clinical impact and optimize team efficiency.</p>
        </div>
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate={isInView ? 'visible' : 'hidden'} className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <motion.div variants={scaleIn} whileHover={{ scale: 1.02, y: -5 }} className="bg-gray-800/80 backdrop-blur-sm border border-white/10 p-10 flex flex-col rounded-3xl shadow-2xl hover:shadow-red-600/10 transition-all">
            ...
            <h3 className="text-2xl font-bold mb-4">Patient Outcomes Dashboard</h3>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.5 }} className="bg-gray-900/80 backdrop-blur-sm border border-white/10 p-6 mb-8 rounded-2xl">
              <div className="flex justify-between items-end mb-2">
                <span className="text-sm font-bold uppercase text-gray-400">Recovery Rate</span>
                <motion.span initial={{ scale: 0 }} animate={isInView ? { scale: 1 } : {}} transition={{ delay: 0.7, type: 'spring' }} className="text-2xl font-bold text-green-500">
                  +12.4%
                </motion.span>
              </div>
              <div className="w-full bg-gray-700 h-1 rounded-full overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={isInView ? { width: '85%' } : {}} transition={{ duration: 1, delay: 0.8 }} className="bg-green-500 h-1" />
              </div>
            </motion.div>
            <a href="#" className="inline-flex items-center font-bold uppercase tracking-wider text-sm text-red-400 hover:text-white transition-colors group">
          ...
          <motion.div variants={scaleIn} whileHover={{ scale: 1.02, y: -5 }} className="bg-gray-800/80 backdrop-blur-sm border border-white/10 p-10 flex flex-col rounded-3xl shadow-2xl hover:shadow-red-600/10 transition-all">
            ...
            <h3 className="text-2xl font-bold mb-4">Care Team Collaboration Hub</h3>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.5 }} className="bg-gray-900/80 backdrop-blur-sm border border-white/10 p-6 mb-8 rounded-2xl">
              <div className="flex -space-x-2 mb-4">
                {[1, 2, 3, 4].map((i) => (
                  <motion.div key={i} initial={{ scale: 0 }} animate={isInView ? { scale: 1 } : {}} transition={{ delay: 0.6 + i * 0.1 }} whileHover={{ scale: 1.2, zIndex: 10 }} className="w-8 h-8 rounded-full bg-gray-700 border-2 border-gray-900 flex items-center justify-center text-xs font-bold text-gray-400 cursor-pointer">
                    U{i}
                  </motion.div>
                ))}
                <motion.div initial={{ scale: 0 }} animate={isInView ? { scale: 1 } : {}} transition={{ delay: 1 }} whileHover={{ scale: 1.2, zIndex: 10 }} className="w-8 h-8 rounded-full bg-red-600 text-white border-2 border-gray-900 flex items-center justify-center text-xs font-bold cursor-pointer">
                  +8
                </motion.div>
              </div>
